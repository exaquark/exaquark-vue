...
var NeighborsSet = {
  ...
  insertOrUpdateNeighbor: function (iid, neighbourState, stream, isPeerAuthority) {
    ...
    if (!newNeighbor && !this.remoteStreamChanged(neighbor, neighbourState)) { // update, no change to peerConnection (or doesn't have webrtc)
  ...
  remoteStreamChanged: function (neighbor, newState) {
    let changed = neighbor.state.customState && neighbor.state.customState.webrtc && (neighbor.state.customState.webrtc.streamId !== newState.customState.webrtc.streamId)
    return changed
  }
}
